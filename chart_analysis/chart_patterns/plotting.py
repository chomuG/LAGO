...
def _add_head_shoulder_pattern_plot(row: Union[tuple, pd.DataFrame], fig: go.Candlestick, 
    ...
    if isinstance(row, tuple):
        ...
        x_values = row[x_val].values[0]
        y_values = row[y_val].values[0]       
    ...
    # Add markers for the 5 specific H&S points
    fig.add_scatter(x = x_values, y = y_values,
                    mode="markers",
...
def _add_doubles_pattern_plot(row: Union[tuple, pd.DataFrame], fig: go.Candlestick) -> go.Candlestick:
    ...
    if isinstance(row, tuple):
        ...
        double_idx = row["double_idx"].values[0]
        double_pts = row["double_point"].values[0] 
...
def _add_triangle_pattern_plot(row: Union[tuple, pd.DataFrame], fig: go.Candlestick) -> go.Candlestick:
    ...
    if isinstance(row, pd.DataFrame):
        ...
        intercmax = row["triangle_intercmax"].values[0]
        slmax     = row["triangle_slmax"].values[0]
...
def display_chart_pattern(ohlc: pd.DataFrame, pattern: str = "flag", 
    ...
    if len(pattern_points) == 0: # There is no pattern found
        ...
        pattern_point = pattern_points.index[0]

        # Make sure at least `lookback` candlesticks are available, if possible
        if pattern_point - lookback < 0:
            start  = 0
        else:
            start = pattern_point - lookback
        
        # Get a subset of the ohlc to "zoom in" on the pattern
        ...
        if save:
            # Pass the row information to save with the correct index
            save_chart_pattern(fig, pattern, (pattern_point, pattern_points.iloc[0]))
        else:
            fig.show()
    elif len(pattern_points) > 1:
