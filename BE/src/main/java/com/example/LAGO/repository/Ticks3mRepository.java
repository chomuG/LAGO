...
public interface Ticks3mRepository extends JpaRepository<Ticks3m, Ticks3mId> {
     ...
     */
    @Query("SELECT t FROM Ticks3m t JOIN t.stockInfo s WHERE s.code = :code AND t.id.bucket >= :startTime AND t.id.bucket < :endTime ORDER BY t.id.bucket ASC")
     ...
     */
    @Query("SELECT t FROM Ticks3m t JOIN t.stockInfo s WHERE s.code = :code ORDER BY t.id.bucket DESC")
