...
public interface Ticks5mRepository extends JpaRepository<Ticks5m, Ticks5mId> {
    ...
    @Query("SELECT t FROM Ticks5m t JOIN t.stockInfo s WHERE s.code = :code AND t.id.bucket >= :startTime AND t.id.bucket < :endTime ORDER BY t.id.bucket ASC")
    ...
    @Query("SELECT t FROM Ticks5m t JOIN t.stockInfo s WHERE s.code = :code ORDER BY t.id.bucket DESC")
