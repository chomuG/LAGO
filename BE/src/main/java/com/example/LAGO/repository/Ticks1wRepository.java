...
public interface Ticks1wRepository extends JpaRepository<Ticks1w, Ticks1wId> {
     ...
     */
    @Query("SELECT t FROM Ticks1w t JOIN FETCH t.stockInfo s WHERE s.code = :code AND t.id.bucket >= :startTime AND t.id.bucket < :endTime ORDER BY t.id.bucket ASC")
     ...
     */
    @Query("SELECT t FROM Ticks1w t JOIN FETCH t.stockInfo s WHERE s.code = :code ORDER BY t.id.bucket DESC")
