...
public interface Ticks1dRepository extends JpaRepository<Ticks1d, Ticks1dId> {
     ...
     */
    @Query("SELECT t FROM Ticks1d t JOIN t.stockInfo s WHERE s.code = :code AND t.id.bucket >= :startTime AND t.id.bucket < :endTime ORDER BY t.id.bucket ASC")
     ...
     */
    @Query("SELECT t FROM Ticks1d t JOIN t.stockInfo s WHERE s.code = :code ORDER BY t.id.bucket DESC")
